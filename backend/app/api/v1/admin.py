...
from app.core.constants import (
    MONGO_LOOKUP, MONGO_MATCH, MONGO_UNWIND, MONGO_GROUP, MONGO_SORT,
    MONGO_SKIP, MONGO_LIMIT, MONGO_SUM
)
...
async def get_dashboard_analytics(
    ...
    try:
        ...
        user_growth = await db.users.aggregate(user_growth_pipeline).to_list(length=None)
        ...
        revenue_trend = await db.bookings.aggregate(revenue_trend_pipeline).to_list(length=None)
        ...
        return StandardResponse(
            ...
            data={
                ...
                "revenue": {
                    ...
                    "platform_fees": revenue_data[0]["platform_fees"] if revenue_data else 0.0
                },
...
async def verify_acharya(  # noqa: C901
    ...
    try:
        ...
        if acharya["status"] == UserStatus.ACTIVE.value:
            raise InvalidInputError(
                message="Acharya is already verified",
                field="acharya_id"
            )
        ...
        notification_message = _create_verification_notification_message(verification.action, verification.notes)
...
async def broadcast_notification(
    ...
    try:
        ...
        logger.info(f"Broadcast notification sent to {len(user_ids)} users by admin {current_user['id']}")

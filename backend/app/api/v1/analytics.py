...
from app.core.constants import (
    MONGO_MATCH, MONGO_GROUP, MONGO_SORT, MONGO_LIMIT,
    MONGO_SUM, MONGO_LOOKUP, MONGO_UNWIND, MONGO_PROJECT
)
...
async def get_analytics_overview(
    time_range: str = Query("30days", description=f"{TIME_RANGE_DESC}: 7days, 30days, 90days, 1year"),
    ...
    try:
        ...
        total_acharyas = await db.users.count_documents({"role": "acharya", "status": "active"})
        ...
        revenue_pipeline = [
            ...
            },
            {
                MONGO_GROUP: {
                    "_id": None,
                    "total": {MONGO_SUM: FIELD_TOTAL_AMOUNT}
                }
            }
        ]
        ...
        revenue_prev_result = await db.bookings.aggregate(revenue_prev_pipeline).to_list(1)
        ...
        return StandardResponse(
            ...
            data={
                ...
                "bookingGrowth": calculate_growth(new_bookings_current, new_bookings_prev)
            }
        )
...
async def get_booking_status(
    ...
    try:
        ...
        status_data = [
            {
                "name": status_names.get(r["_id"], r["_id"].title()),
                "value": r["count"]
            }
            for r in results
        ]
        ...
        pipeline = [
            ...
            },
            {
                "$addFields": {
                    ...
                    "total_revenue": {
                        "$sum": "$completed_bookings.acharya_amount"
                    }
                }
            },
...
async def get_geographic_distribution(
    ...
    try:
        group_field = f"$address.{group_by}" if group_by in ["city", "state"] else "$address.city"
    ...
    try:
        ...
        funnel = [
            {"stage": "Signup", "users": signups, "rate": 100},
            {
                ...
                "rate": round((profiles_completed / signups * 100) if signups else 0, 1)
            },
            {
                "stage": "Made Booking",
                "users": made_booking,
        ...
        pipeline = [
            {MONGO_MATCH: match_stage} if match_stage else {MONGO_MATCH: {}},
            {
                MONGO_GROUP: {
                    ...
                    "total": {MONGO_SUM: 1},
                    "completed": {
                        MONGO_SUM: {MONGO_COND: [{"$eq": [FIELD_STATUS, "completed"]}, 1, 0]}
                    },
                    "cancelled": {
                        MONGO_SUM: {MONGO_COND: [{"$eq": [FIELD_STATUS, "cancelled"]}, 1, 0]}
                    },
                    "total_revenue": {
                        MONGO_SUM: {
                            MONGO_COND: [
    ...
    try:
        ...
        for cohort in cohorts:
            ...
            active_users = await db.bookings.distinct(
                "grihasta_id",
                {"grihasta_id": {"$in": cohort["user_ids"]}}
            )
            
            retention_rate = (len(active_users) / cohort["total_users"] * 100) if cohort["total_users"] else 0

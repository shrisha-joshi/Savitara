...
# Constants
INVALID_ACHARYA_ID_MSG = "Invalid acharya_id"
...
async def get_acharya_schedule(
    ...
    try:
        ...
        if not ObjectId.is_valid(acharya_id):
            raise HTTPException(status_code=400, detail=INVALID_ACHARYA_ID_MSG)
...
async def update_acharya_schedule(
    ...
    try:
        ...
        if (
            str(current_user.get("_id")) != acharya_id
        ...
        schedule_doc = {
            "acharya_id": ObjectId(acharya_id),
            "date": schedule_data.date.replace(
...
async def create_grihasta_event(
    ...
    try:
        ...
        event_doc = {
            ...
            "booking_id": ObjectId(event_data.booking_id)
...
async def update_grihasta_event(
    ...
    try:
        ...
        if result.matched_count == 0:
            raise HTTPException(
                status_code=404, detail="Event not found or not authorized"
            )

        return StandardResponse(
            success=True,
            message="Event updated successfully",
            data={"event_id": event_id, "modified": result.modified_count > 0},
        )
...
async def get_upcoming_events(
    ...
    try:
        ...
        events = (
            await db.grihasta_calendar_events.find(
                {
                    "user_id": ObjectId(user_id),
        ...
        return StandardResponse(
            ...
            data={"events": events, "count": len(events), "period_days": days},
        )

    except Exception as e:
        logger.error(f"Error fetching upcoming events: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch upcoming events: {str(e)}"
        )

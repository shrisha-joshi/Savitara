...
async def verify_payment(
    ...
    try:
        ...
        if not is_valid:
             raise PaymentFailedError(order_id=request.razorpay_order_id, details={"reason": "Invalid Signature"})
...
async def initiate_refund(
    ...
    await db.payments.update_one(
        ...
        {"$set": {"status": "refunded", "refund_processed_at": datetime.now(timezone.utc)}}
    )

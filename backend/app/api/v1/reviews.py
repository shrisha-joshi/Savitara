...
from app.schemas.requests import ReviewCreateRequest, StandardResponse
    ...
    try:
        ...
        if not booking:
            raise ResourceNotFoundError(
                resource_type="Booking", resource_id=review_data.booking_id
            )
        ...
        if booking["status"] != BookingStatus.COMPLETED.value:
            raise InvalidInputError(
                message="Can only review completed bookings", field="booking_id"
            )
        ...
        logger.info(
            f"Review submitted by {grihasta_id} for booking {review_data.booking_id}"
        )
...
async def get_acharya_reviews(
    ...
    try:
        # Build query filter
        query = {"acharya_id": acharya_id, "is_public": True}
        ...
        return StandardResponse(
            ...
            data={
                "reviews": reviews,
                "stats": {
                    "average_rating": round(rating_stats[0]["average_rating"], 2)
                    if rating_stats
                    else 0.0,
                    "total_reviews": rating_stats[0]["total_reviews"]
                    if rating_stats
                    else 0,
                },
                "pagination": {
...
async def get_pooja_reviews(
    ...
    try:
        # Get reviews
        query = {"pooja_id": pooja_id, "review_type": "pooja", "is_public": True}
...
async def update_review(
    ...
    try:
        ...
        if review["is_public"]:
            raise InvalidInputError(
                message="Cannot update review after admin approval", field="review_id"
            )
        ...
        if not update_fields:
            raise InvalidInputError(message="No fields to update", field="body")

        update_fields["updated_at"] = datetime.now(timezone.utc)

        # Update review
        await db.reviews.update_one({"_id": review_id}, {"$set": update_fields})
...
async def get_platform_stats(db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    try:
        ...
        platform_pipeline = [
            {"$match": {"review_type": "platform", "is_public": True}},
            {
                MONGO_GROUP: {
                    "_id": None,
                    "average_rating": {MONGO_AVG: FIELD_RATING},
                    "total_reviews": {MONGO_SUM: 1},
                    "rating_1": {
                        MONGO_SUM: {MONGO_COND: [{MONGO_EQ: [FIELD_RATING, 1]}, 1, 0]}
                    },
                    "rating_2": {
                        MONGO_SUM: {MONGO_COND: [{MONGO_EQ: [FIELD_RATING, 2]}, 1, 0]}
                    },
                    "rating_3": {
                        MONGO_SUM: {MONGO_COND: [{MONGO_EQ: [FIELD_RATING, 3]}, 1, 0]}
                    },
                    "rating_4": {
                        MONGO_SUM: {MONGO_COND: [{MONGO_EQ: [FIELD_RATING, 4]}, 1, 0]}
                    },
                    "rating_5": {
                        MONGO_SUM: {MONGO_COND: [{MONGO_EQ: [FIELD_RATING, 5]}, 1, 0]}
                    },
                }
            },
        ]
        ...
        return StandardResponse(
            success=True,
            data={
                "platform_reviews": platform_stats[0]
                if platform_stats
                else {
                    "average_rating": 0.0,
                    "total_reviews": 0,
                ...
                "overall_stats": {
                    "total_reviews": overall_stats[0]["total_reviews"]
                    if overall_stats
                    else 0,
                    "average_rating": round(overall_stats[0]["average_rating"], 2)
                    if overall_stats
                    else 0.0,
                },
            },

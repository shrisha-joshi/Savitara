...
from app.models.services import (
    Service, ServiceBooking, ServiceCategory, 
    ServiceBookingType, MuhurtaRequired
)
...
async def get_services(
    ...
    try:
        ...
        services = await db.services.find(query).sort("popularity_score", -1).skip(skip).limit(limit).to_list(length=limit)
...
async def get_my_service_bookings(
    ...
    try:
        ...
        bookings = await db.service_bookings.find(query).sort("created_at", -1).to_list(length=100)
...
async def create_service_booking(
    ...
    try:
        ...
        if booking_request.booking_type not in [bt.value for bt in ServiceBookingType]:
            raise InvalidInputError(message="Invalid booking type", field="booking_type")
        
        # Calculate pricing based on booking type
        if booking_request.booking_type == ServiceBookingType.MUHURTA_CONSULTATION.value:
        ...
        booking = {
            ...
            "muhurta_details": booking_request.muhurta_details.dict() if booking_request.muhurta_details else {},
            "acharya_id": ObjectId(booking_request.acharya_id) if booking_request.acharya_id else None,
            "is_platform_assigned": booking_request.acharya_id is None,
            "venue_address": booking_request.venue_address.dict() if booking_request.venue_address else {},
        ...
        logger.info(f"Service booking created: {result.inserted_id} by user {current_user['id']}")
...
async def cancel_service_booking(
    ...
    try:
        ...
        if booking["status"] in ["completed", "cancelled"]:
            raise InvalidInputError(message="Cannot cancel this booking", field="status")

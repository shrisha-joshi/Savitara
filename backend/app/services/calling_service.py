...
class CallingService:
    ...
    async def initiate_masked_call(
        self, caller_user_id: str, callee_user_id: str, booking_id: str, db
    ) -> dict:
        """
        ...
        caller_profile = await db.grihasta_profiles.find_one(
            {"user_id": caller_user_id}
        )
        if not caller_profile:
            caller_profile = await db.acharya_profiles.find_one(
                {"user_id": caller_user_id}
            )

        callee_profile = await db.acharya_profiles.find_one({"user_id": callee_user_id})
        if not callee_profile:
            callee_profile = await db.grihasta_profiles.find_one(

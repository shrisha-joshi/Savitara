...
from app.models.gamification import (
    CoinTransaction, UserCoins, PointsTransaction, UserLoyalty,
    Voucher, UserVoucher, Coupon, CouponUsage, PricingRule, PriceDisplay,
    Referral, DailyReward, UserStreak, Milestone,
    ActionType, RewardType, LoyaltyTier, AcharyaTier,
    COIN_REWARDS, POINTS_REWARDS, LOYALTY_TIERS, ACHARYA_TIERS,
    COINS_TO_RUPEES, MAX_COIN_REDEMPTION_PERCENT
)
...
class GamificationService:
    ...
    async def award_coins(
        ...
        await self.coin_transactions.insert_one(transaction.dict(by_alias=True, exclude={"id"}))
    ...
    async def redeem_coins(
        ...
        # Validate amount
        max_redemption = int(booking_amount * MAX_COIN_REDEMPTION_PERCENT / 100 / COINS_TO_RUPEES)
    ...
    async def get_user_coins(self, user_id: str) -> Dict:
        ...
        transactions = await self.coin_transactions.find(
            {"user_id": user_id}
    ...
    async def award_points(
        ...
        if not loyalty:
            ...
            loyalty = {
                "user_id": user_id,
                "role": role,
                "current_tier": LoyaltyTier.BRONZE.value if role == "grihasta" else AcharyaTier.RISING_STAR.value,
                "points": 0,
                "tier_progress": 0.0,
        ...
        await self.points_collection.update_one(
            {"user_id": user_id},
            {
                ...
                "$set": {
                    ...
                    "discount_percentage": tier_data.get("discount", tier_data.get("commission_reduction", 0)),
        ...
        await self.points_transactions.insert_one(transaction.dict(by_alias=True, exclude={"id"}))
    ...
    async def get_user_loyalty(self, user_id: str) -> Dict:
        ...
        return {
            **loyalty,
            ...
            "tier_benefits": self._get_tier_benefits(loyalty["current_tier"], loyalty["role"])
        }
    ...
    def _get_tier_benefits(self, tier: str, role: str) -> List[str]:
        ...
        if role == "grihasta":
            benefits = {
                ...
                "platinum": ["20% discount", "Free cancellations", "5x coins", "VIP support"]
            }
        else:
            benefits = {
                ...
                "guru": ["Homepage feature", "10% commission reduction", "Personal manager"]
            }
    ...
    async def create_voucher(self, voucher_data: Dict) -> Dict:
        ...
        result = await self.vouchers.insert_one(voucher.dict(by_alias=True, exclude={"id"}))
    ...
    async def award_voucher_to_user(
        ...
        # Get voucher
        voucher = await self.vouchers.find_one({"code": voucher_code, "is_active": True})
        ...
        await self.user_vouchers.insert_one(user_voucher.dict(by_alias=True, exclude={"id"}))
    ...
    async def calculate_price(
        ...
        coupon_discount = 0
        if coupon_code:
            validation = await self.validate_coupon(coupon_code, user_id, base_amount, service_id)
        ...
        total_discount = discount_amount + tier_discount + coupon_discount + coin_discount
    ...
    async def create_referral(
        ...
        result = await self.referrals.insert_one(referral.dict(by_alias=True, exclude={"id"}))
    ...
    async def complete_referral(
        ...
        referral_id: str,
        referee_id: str,
        first_booking_id: Optional[str] = None
    ) -> Dict:
        ...
        await self.referrals.update_one(
            {"_id": ObjectId(referral_id)},
            {
                "$set": {
                    ...
                    "first_booking_at": datetime.now(timezone.utc) if first_booking_id else None,
                    "rewards_given": True,
        ...
        await self.award_coins(
            ...
            ActionType.REFER_FRIEND if not first_booking_id else ActionType.REFERRAL_BOOKING,

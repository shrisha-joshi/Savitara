...
class QueryOptimizer:
    ...
    async def create_all_indexes(self):
        ...
        index_definitions = [
            ...
            (
                "users",
                [("phone", 1)],
                {"unique": True, "sparse": True, "name": "phone_unique_idx"},
            ),
        ...
        for collection_name, index_spec, options in index_definitions:
            try:
                ...
                logger.info(
                    f"Created index on {collection_name}: {index_spec} -> {result}"
                )
                created_count += 1
            except Exception as e:
                if "already exists" not in str(e):
                    logger.error(
                        f"Failed to create index on {collection_name} {index_spec}: {e}"
                    )
                    failed_count += 1

        logger.info(
            f"Index creation complete: {created_count} created, {failed_count} failed"
        )
    ...
    async def get_acharyas_optimized(
        ...
        try:
            results = await self.db.acharya_profiles.aggregate(pipeline).to_list(
                length=limit
            )
            return results
        except Exception as e:
    ...
    async def analyze_query_performance(
        ...
        try:
            ...
            return {
                ...
                "index_used": explanation.get("queryPlanner", {})
                .get("winningPlan", {})
                .get("inputStage", {})
                .get("indexName"),
                "efficient": exec_stats.get("totalDocsExamined", 0)
                <= exec_stats.get("nReturned", 0) * 1.2,  # Within 20% overhead
            }
        except Exception as e:
    ...
    async def get_slow_queries_report(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            slow_queries = (
                await self.db.system.profile.find({"millis": {"$gt": 100}})
                .sort("millis", -1)
                .limit(20)
                .to_list(length=20)
            )

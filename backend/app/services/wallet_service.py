...
class WalletService:
    ...
    async def add_bonus(
        ...
        transaction = {
            ...
            "description": reason,
            "status": TransactionStatus.COMPLETED.value,
            "expiry_date": datetime.now(timezone.utc) + timedelta(days=expiry_days) if expiry_days else None,
            "metadata": metadata or {},
            "created_at": datetime.now(timezone.utc)
        }
    ...
    async def process_refund(
        ...
        return {
            ...
            "new_balance": transaction["balance_after"] if not refund_to_source else wallet["balance"],
    ...
    async def get_transaction_history(
        ...
        cursor = self.transactions.find(query).sort("created_at", -1).skip(offset).limit(limit)
...
# Import for expiry_days calculation

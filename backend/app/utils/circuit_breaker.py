...
class CircuitBreaker:
    ...
    def _record_failure(self, exception: Exception):
        ...
        if self._state == CircuitState.HALF_OPEN:
            ...
            logger.warning(
                f"Circuit '{self.name}' re-opening after failure in half-open"
            )
...
payment_circuit = CircuitBreaker(
    name="payment_gateway", failure_threshold=3, reset_timeout=30.0, timeout=15.0
)

notification_circuit = CircuitBreaker(
    name="notification_service", failure_threshold=5, reset_timeout=60.0, timeout=10.0
)

email_circuit = CircuitBreaker(
    name="email_service", failure_threshold=5, reset_timeout=120.0, timeout=30.0
)

sms_circuit = CircuitBreaker(
    name="sms_service", failure_threshold=5, reset_timeout=60.0, timeout=10.0
)

...
class InputSanitizer:
    ...
    @staticmethod
    def _sanitize_list(
        items: List[Any], fields_to_sanitize: List[str] = None
    ) -> List[Any]:
        """Sanitize items in a list"""
        return [
            InputSanitizer._sanitize_value(item, fields_to_sanitize) for item in items
        ]

    @staticmethod
    def sanitize_dict(
        data: Dict[str, Any], fields_to_sanitize: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for key, value in data.items():
            ...
            sanitized[key] = (
                InputSanitizer._sanitize_value(value, fields_to_sanitize)
                if should_sanitize
                else value
            )
...
class TokenBlacklist:
    ...
    async def get_user_token_invalidation_time(
        self, user_id: str
    ) -> Optional[datetime]:
        """
